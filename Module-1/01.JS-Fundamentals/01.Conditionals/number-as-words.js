...
const convertNumber = (args) => {
    ...
    for (i = 0; i < args.length; i += 1) {
        if (+args[i] < 20) {
            ...
            result =
                tens[Math.floor(args[i] / 10)].trim()
                + ones[+args[i] % 10].toLowerCase();
        } else if ((args[i] % 100) === 0 && args[i] > 99 && args[i] < 1000) {
            result = ones[args[i] / 100] + ' hundred';
        } else if ((args[i] % 100) !== 0
            && args[i] > 99
            && args[i] < 1000
            && +args[i] % 10 !== 0) {
            result =
                ones[Math.floor(args[i] / 100)]
                + ' hundred and';
            if (args[i] % 100 < 20) {
            ...
            result =
                ones[Math.floor(args[i] / 100)]
                + ' hundred and'
                + tens[Math.floor(args[i] / 10) % 10].toLowerCase();
        }
    }
    return result;
};
