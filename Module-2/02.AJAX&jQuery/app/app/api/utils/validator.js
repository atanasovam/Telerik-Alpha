...
const isNameValid = (name) => {
    ...
    if (name.length < NAME_MIN_LENGTH &&
        ...
        return {
            isValid: false,
            error: `Name length must be between ` +
...
const isSecretIdentityValid = (secretIdentity) => {
    ...
    if (secretIdentity.length < SECRET_IDENTITY_MIN_LENGTH &&
        ...
        return {
            isValid: false,
            error: `Secret Identity's length must be between ` +
                `${SECRET_IDENTITY_MIN_LENGTH} and ` +
                `${SECRET_IDENTITY_MAX_LENGTH} ` +
                ` characters long`,
        };
    }
...
const validator = {
    isValid(superhero) {
        ...
        return isNameValid(superhero.name) &&
            isSecretIdentityValid(superhero.secretIdentity);
    },
};
